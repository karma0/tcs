...
const int16_t sine_table[256] = {
     0,    9,   18,   26,   35,   44,   53,   61,
    70,   78,   87,   95,  104,  112,  121,  129,
   137,  145,  153,  161,  169,  176,  184,  192,
   199,  206,  213,  220,  227,  234,  240,  247,

   253,  259,  265,  271,  277,  282,  288,  293,
   298,  302,  307,  311,  316,  320,  324,  327,
   331,  334,  337,  340,  343,  345,  347,  349,
   351,  353,  354,  355,  356,  357,  358,  358,

   358,  358,  358,  357,  356,  355,  354,  353,
   351,  349,  347,  345,  343,  340,  337,  334,
   331,  327,  324,  320,  316,  311,  307,  302,
   298,  293,  288,  282,  277,  271,  265,  259,

   253,  247,  240,  234,  227,  220,  213,  206,
   199,  192,  184,  176,  169,  161,  153,  145,
   137,  129,  121,  112,  104,   95,   87,   78,
    70,   61,   53,   44,   35,   26,   18,    9,
...
// timer flag is set every 16us
volatile uint8_t timer_flag = 0;
// counter_a and counter_b are incremented every 16us
...
Adafruit_RGBLCDShield lcd = Adafruit_RGBLCDShield(); // 8, 9, 4, 5, 6, 7);
...
void DACwrite(uint16_t value, uint8_t cs) {
  digitalWrite(cs, LOW);
  SPI.transfer16(value);
  digitalWrite(cs, HIGH);
}

//When timer2 overflows,
ISR(TIMER2_OVF_vect) {
  timer_flag = 1;
  counter_a++;
  counter_b++;
...
void UpdateScreen(void) {
  ...
  } else if (whichscreen == 1) {  // Channel B screen
...
void loop() {
  ...
  if (whichmode == 0) {//normal mode
    ...
    if ((burst_on == 1 && millis_now - prev_millis >= burst_ms_on)
        || (burst_on == 0 && millis_now - prev_millis >= burst_ms_off)) {
      burst_on ^= 0x01;
